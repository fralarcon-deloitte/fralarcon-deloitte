CLASS zcl_fdac_renting_load_data DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_fdac_renting_load_data IMPLEMENTATION.


  METHOD if_oo_adt_classrun~main.
    DATA: LT_BRANDS TYPE TABLE OF zfd_rent_brands,
          LT_CARS TYPE TABLE OF zfd_rent_cars,
          LT_CUSTOMERS TYPE TABLE OF zfd_rent_costum.
*   * BRANDS **
    lt_brands = VALUE #(
    ...
    DELETE FROM zfd_rent_brands.
    INSERT zfd_rent_brands FROM TABLE @lt_brands.

    SELECT * FROM zfd_rent_brands INTO TABLE @lt_brands.

    out->write( sy-dbcnt ).
    out->write( 'zfd_rent_brands data inserted successfully!').
...
*   * CARS**
    ...
    DELETE FROM zfd_rent_cars.
    INSERT zfd_rent_cars FROM TABLE @lt_cars.

    SELECT * FROM zfd_rent_cars INTO TABLE @lt_cars.
    out->write( sy-dbcnt ).
...
*    * CUSTOMERS**
    lt_customers = VALUE #(
        ...
        DELETE FROM zfd_rent_costum.
        INSERT zfd_rent_costum FROM TABLE @lt_customers.

        SELECT * FROM zfd_rent_costum INTO TABLE @lt_customers.
        out->write( sy-dbcnt ).
